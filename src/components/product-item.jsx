...
export default class ProductItem extends React.Component {
  ...
  render() {
    var itemLink = `#/product/${this.state.product.id}`;
    ...
    var wishListButton = <Button variant="link" bottom="true" onClick={() => this.addToWishlist()}><FontAwesomeIcon icon={faHeart} size="2x" color="#1551d1e3" /></Button>
    if (this.state.itemInWishlist === true) {
      wishListButton = <Button variant="link" bottom="true" onClick={() => this.removeFromWishlist()}><FontAwesomeIcon icon={faHeartBroken} size="2x" color="#1551d1e3" /></Button>
    }

    return (
      <Col>
        <Card style={{ marginBottom: "20px" }} >
          <Card.Img top="true" width="100%" data-src="holder.js/100px150" alt={this.state.product.name} />
